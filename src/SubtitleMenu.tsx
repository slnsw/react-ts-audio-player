...
const SubtitleMenu: React.FunctionalComponent<Props> = ({ visible, id, subtitleTracks, selected, onSelect }) => {};
...
module.exports = class OhSubtitleMenu extends Component {
    ...
    for (i = 0; i < this.props.subtitleTracks.length; i++) {
      ...
      options.push(
        <li key={ `${track.language}-${i}` }>
          { this.button(track.language, track.label, track.language == this.props.selected) }
        </li>
      );
    }
    return options;
  ...
  render(props, {}) {
    return (
      <ul class="video-wrapper__popup-menu subtitles-menu"
          id={ props.id }
